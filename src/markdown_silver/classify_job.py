...
def main() -> None:
    ...
    needs_refresh = (
        (F.col("classification_status") == "PENDING") |
    ...
    output_schema = (
        "STRUCT<section_type:STRING,subject:STRING,topic:STRING,"
        "difficulty:STRING,language:STRING,confidence:DOUBLE>"
    )
    ...
    classified = (queried
      .select("section_id", "raw_result.response.*", "raw_result.errorMessage")
